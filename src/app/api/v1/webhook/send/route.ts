...
export async function POST(req: NextRequest) {
  const guarded = await channelGuard(req, 'webhook');
  if (!guarded.ok) return guarded.response;
  const body = await parseJson<{ target?: unknown; payload: unknown; externalId?: string; priority?: 'low' | 'normal' | 'high' | 'critical'; tags?: string[] }>(req);
